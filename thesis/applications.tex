...
using \fn{collectTypes}. We know that there is at least one member of that
list, so we name the last element \bn{retTy}, for the return type, and we name
the rest \bn{argTy}, since they represent types of the arguments.
For each of member of \bn{argTy}, we generate a new name using

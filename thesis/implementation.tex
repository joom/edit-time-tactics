...
\begin{enumerate}
  ...
  \item Now we have \bn{name}, yet we do not know whether it really refers to a
    ...
    We lookup the context and find out its type, which we can call \bn{ty}, and
    that has the Haskell type \ty{Type}. From the context, we also resolve the
  ...
  \item We zip \bn{collected} and \bn{args} and get a list of pairs of
    ...
    each pair of \texttt{(\bn{argTy}, \bn{sexp})}:
    \begin{enumerate}[(a)]
      ...
      \item Since \bn{tm} is the Haskell representation of an \Elab\ action,
        we should run it using \fn{runElabAction}. This gives us \bn{tm'}, the
  ...
  \item We want to serialize this term before returning it as an S-expression.
    ...
    in \ty{Editorable}, but remember that that is defined in Idris.
...
\section{Extensions to the type-checker for \kw{\%editor}}\label{sec:extCheck}
...
\fn{toy} example in \autoref{ssec:primitiveEditorableDesign}, and also have

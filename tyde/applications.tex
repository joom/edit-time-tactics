...
An eDSL that most programmers are already familiar with is regular expressions.
...
regexes has constructors for $\varnothing$, $\varepsilon$, a literal
character, concatenation ($\cdot$), alternation ($+$) and the Kleene star ($*$),
...
many external sources such as \citet{regexSimplification} that do, and it is
not essential to understand how the custom editor action works.
We will take the function \mt{\fn{simplify} : \ty{Regex} -> \ty{Regex}} as a
given, and proceed to explain how it can be used to construct an editor

...
language terms into the high-level language terms. Then, the compiler should invoke the pretty
...
\end{Verbatim}
  \caption{An \Editorable{} instance depending on the new primitives.}\label{fig:editorablePrim}
\end{figure}

\subsection{How the compiler uses \Editorable{} for communication}
\label{ssec:communication}

We have extended Idris's IDE protocol to support an additional message that represents an invocation of a custom editor action.
This message includes the name of the custom action and a list of arguments, and its reply contains Idris's response.

When a custom editor action message is received, \todo{What happens? Idris looks it up in a table of \%editor actions, maps fromEditor over the arguments, checks the arity, then calls it in Elab, then serializes the answer with toEditor, and sends it back?}



As an example, the editor action \fn{easy} from figures \ref{fig:motivating-example} and \ref{fig:motivating-example-exec},\todo{Connect me to the prior discussion}
presented a simple editor action \fn{easy}, its supporting Emacs Lisp code,
...
\fn{easy} only takes a single argument of the type \ty{TTName}. \TODO{this is hard to write without shifting to the tutorial style - DC: I added a prototype for you!}
...
When Idris encounters a definition that is tagged as an editor action, it first elaborates the type.
The next step is to check whether this type is suitable as an editor action.
It does this by ensuring that each argument type is Editorable, that the return type has \Elab{} at its head, and that the argument to \Elab{} is also Editorable.
Note that this rules out dependent types for editor actions---section~\todo{refer to future work} discusses a potential way to lift this restriction in the future.

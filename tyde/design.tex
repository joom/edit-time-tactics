...
\TODO{This section needs to be simplified quite a bit more. Some parts of it
can be moved to the implementation section. Also there must be a better way to
motivate for \ty{Editorable} without showing all the S-expressions. (?)}
...
function clause, the editor sends the message \mt{((:add-clause 4 "height") 8)}
to the compiler, after doing the necessary
computation, the compiler responds with the message \mt{(:return (:ok "height t1 = ?height\_rhs") 8)}
...
case-splitting action.  When we place the cursor on \bn{t1} and run the
case-splitting action, the compiler sends \mt{((:case-split 5 "t1") 11)} and receives
\begin{Verbatim}
(:return (:ok
...
Suppose we have an \Elab\ action \mt{\fn{prover} : \ty{TTName} -> \ty{Elab}
\ty{TT}}, which takes a hole name, and gives back a \TT\ term in the \Elab\
monad. As you can tell from the name \fn{prover}, we intend this action to
...
We can achieve this by sending \mt{((:elab-edit "prover" '("q") 21 5) 22)}.
Then we expect to receive a response like \mt{(:return (:ok "Left ()") 22)}.
...
\TODO{I'm not sure if we need to give this implementation definition}
...
\subsubsection{Primitive \ty{Editorable} implementations}\label{ssec:primitiveEditorableDesign}
...
\subsubsection{Using \ty{Editorable} in the compiler and the \kw{\%editor} modifier}\label{ssec:usingEditorable}
...
\TODO{We might want to find a more compelling \fn{toy} example. I picked the one I have below because it has more than one argument, but it doesn't do anything interesting. What example can we give that is simple enough to be implemented in a few lines, has multiple arguments, and does something interesting. Maybe we can tweak the \fn{trivial} example we have in the beginning somehow? }
...
\subsection{Examples in action}\label{sec:designExample}
...
background, the editor sends \mt{((:elab-edit "toy" ("q" "S Z") 35 5) 20)} to
the compiler and receives \mt{(:return (:ok "1" nil) 20)} as a response.
...
When we try to compile \fn{poly}, we get an error that says there is no \ty{Editorable} implementation for \ty{Type}.
...
When we try to compile \fn{funAction}, we get an error message that says there
is no \ty{Editorable} implementation for \mt{\ty{Nat} -> \ty{Nat}}.
